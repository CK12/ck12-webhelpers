DROP TRIGGER IF EXISTS taxonomy.T_Domaineighbors_create;
...
FOR EACH ROW BEGIN
    IF NEW.parentID IS NOT NULL THEN
        INSERT INTO flx2.BrowseTerms (name, handle, encodedID, termTypeID, parentID)
            SELECT NEW.name, NEW.handle, NEW.encodedID, 4, b.id
            FROM taxonomy.ConceptNodes c, flx2.BrowseTerms b
            WHERE c.id = NEW.parentID AND c.encodedID = b.encodedID
            ON DUPLICATE KEY UPDATE name=VALUES(name), handle=VALUES(handle), parentID=VALUES(parentID);
    ELSE
        SELECT s.name, s.shortname INTO @parentName, @parentEID FROM taxonomy.Subjects s, taxonomy.ConceptNodes c WHERE s.id=c.subjectID AND c.encodedID=NEW.encodedID;
        INSERT IGNORE INTO flx2.BrowseTerms (name, handle, encodedID, termTypeID, parentID) VALUES (@parentName, REPLACE(@parentName, ' ', '-'), @parentEID, 4, null);
        SELECT id INTO @parentID FROM flx2.BrowseTerms WHERE encodedID=@parentEID;
        SELECT b.name, b.shortname INTO @parentName, @parentEID2 FROM taxonomy.Branches b, taxonomy.ConceptNodes c WHERE b.id=c.branchID AND c.encodedID=NEW.encodedID;
        INSERT IGNORE INTO flx2.BrowseTerms (name, handle, encodedID, termTypeID, parentID) VALUES (@parentName, REPLACE(@parentName, ' ', '-'), CONCAT(@parentEID, '.', @parentEID2), 4, @parentID);
        SELECT b.id INTO @parentID FROM flx2.BrowseTerms b WHERE b.encodedID=SUBSTRING_INDEX(NEW.encodedID, '.', 2);
        INSERT INTO flx2.BrowseTerms (name, handle, encodedID, termTypeID, parentID) VALUES (NEW.name, NEW.handle, NEW.encodedID, 4, @parentID)
            ON DUPLICATE KEY UPDATE name=VALUES(name), handle=VALUES(handle), parentID=VALUES(parentID);
    END IF;
END
|
...
FOR EACH ROW BEGIN
    IF NEW.parentID IS NOT NULL THEN
        ...
        UPDATE flx2.BrowseTerms SET name=NEW.name, handle=NEW.handle, parentID=@parentID
...
DROP TRIGGER IF EXISTS taxonomy.T_DomainNeighbors_delete;
DELIMITER |
CREATE TRIGGER taxonomy.T_DomainNeighbors_delete AFTER DELETE ON taxonomy.ConceptNodeNeighbors
FOR EACH ROW BEGIN
    SELECT b.id INTO @reqID FROM flx2.BrowseTerms b, taxonomy.ConceptNodes c WHERE c.id=OLD.requiredConceptNodeID AND b.encodedID=c.encodedID;
    DELETE FROM flx2.DomainNeighbors 
    WHERE domainID=(SELECT b.id FROM flx2.BrowseTerms b, taxonomy.ConceptNodes c WHERE c.id=OLD.conceptNodeID AND b.encodedID=c.encodedID)
