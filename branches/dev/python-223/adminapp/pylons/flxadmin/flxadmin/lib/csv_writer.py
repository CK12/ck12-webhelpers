...
class CsvHandler():
    def __init__ (self):
        self.workdir = config.get('cache_share_dir_csv') + '/csv'
        self.file_name = ""
        self.file_path = ""
        self.headers = []
        self.data =[]
    ...
    def write_csv(self,file_name=None):
        try:
            ...
            for row_data in self.data:
                this_row = dict( (n,  unicode(row_data[n]['description']).encode('utf-8') if n == 'role' else unicode(row_data[n]).encode('utf-8')) for n in self.headers)
