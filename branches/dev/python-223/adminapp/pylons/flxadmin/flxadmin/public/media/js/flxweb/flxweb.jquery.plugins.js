...
$.fn.ck12_smartForm = function(options){
    ...
    function showMessage(obj,valid,message) {
        ...
        if (valid) {
            ...
            if (message) {
                $('.error[data-for="'+id+'"]').html(message);
            }
    ...
    $.validator.addMethod("simple_email", function(value, element) {
        return /(^[-!#$%&'*+/=?^_`{}|~0-9A-Z]+(\.[-!#$%&'*+/=?^_`{}|~0-9A-Z]+)*|^"([\001-\010\013\014\016-\037!#-\[\]-\177]|\\[\001-011\013\014\016-\177])*")@(?:[A-Z0-9]+(?:-*[A-Z0-9]+)*\.)+[A-Z]{2,6}$/i.test(value);
    ...
    $.validator.addMethod("ck12_username",function(value,element) {
        return /^$|^[a-zA-Z]+([a-zA-Z0-9]{4,126})$/.test(value);
    }, "Username should contain characters and numbers only");
