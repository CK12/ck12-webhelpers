    ...
    tinymce.create('tinymce.plugins.ElementBoxPlugin', {
        init : function(ed, url) {
            ...
            ed.addCommand('mceDeleteElementBox', function() {
                ...
                var eb = null;
                if(ed.dom.hasClass(el, 'x-ck12-element-box-placeholder')) {
                    eb = el;
                } else {
                    eb = ed.dom.getParent(ed.selection.getNode(), function(n) {
                        return ed.dom.hasClass(n, 'x-ck12-element-box-placeholder');
                    });
                }
                if(eb) {
            ...
            ed.onInit.add(function() {
                if(ed && ed.plugins.contextmenu) {
                    ed.plugins.contextmenu.onContextMenu.addToTop(function(th, m, e) {
                        ...
                        var eboxTable = ed.dom.getParent(e, 'table');
                        if(eboxTable && ed.dom.hasClass(eboxTable, 'x-ck12-element-box-placeholder')) {

                            m.addSeparator();
                            //Fix for adding option image. http://www.tinymce.com/forum/viewtopic.php?pid=41952#p88239
                            m.add({
                                title : 'Delete Element Box',
                                cmd : 'mceDeleteElementBox',
                                icon: 'elementBoxDelete',
                                ui : true
                            });
                        }
                    });
            ...
            ed.onKeyPress.addToTop(function(ed, e) {
                var key = e.keyCode;
                // capturing double enter key press. 
				var doubleenter = false;
                if(keypresshistory === 13){
                	doubleenter = true;
                }
                if(key === 13){
                		keypresshistory = 13;
                ...
                if(key == 13) {
                    ...
                    if(ec) {
                        ...
                        if(ec) {
                        		if(doubleenter) return t._overrideDoubleEnter(t, e);
                        }
                    }
                }
            });
