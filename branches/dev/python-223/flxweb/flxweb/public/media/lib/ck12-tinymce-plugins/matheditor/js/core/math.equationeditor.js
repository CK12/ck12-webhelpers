			...
			function EqEditor(){
			    ...
			    function confirmClear(){
			    	...
			    	if($formulaRow.css('display')=='none'){
				    	...
				    	$(frameElement).css('height','640px').parents("#"+frameElement.id.slice(0,-4)).css('height','669px');
				    }
		    		...
		    		if ($("#texoutput").attr("data-paste")) {
			        	   //$("#editableMath").hide();
			        	   $("#editableMath").addClass('hideInput');
	    			}
	    			//$("#editableMath").show();
	    			$("#editableMath").removeClass('hideInput');
			    }
			    /**
				...
				*/
			    ...
			    function editLatex(){
			          ...
			          if(!$($(".formula_container").children()).hasClass("paste-option")){
			        	  $(".formula_container").append("<span class='paste-option'>Equations entered in LaTeX cannot be edited in the WYSIWYG editor. Please use the area below to edit.</span>")
			    ...
			    function bindevents() {
			        ...
			        $('#texoutput').off('click.setCaretClick').on('click.setCaretClick', function () {
			        	$('.rendered-math input').each(function () {
				            ...
				            if (pos >= cursorPosition) {
				            	...
				            	if($('#texoutput').val() == $(newInputText).val()){
				            		caretPosition = cursorPosition;
				            	}
				            	else {
				            		caretPosition = pos-cursorPosition;
				            	}
					            $(newInputText).caret({
						            start: caretPosition,
						            end: caretPosition
						        });
					            positionLogger.start = caretPosition;
				                positionLogger.end = caretPosition;
					            $('#texoutput').focus();
					            return false;
				            }
				        });
			        ...
			        $('#texoutput').keydown(function (event) {
			            var textareaCursorPosition = $('#texoutput').caret().start,
			            	...
			            	variableRemoved = 0;
			            setTimeout(function () {
			            	if((key!=17 && key!=16 && key!=9 && event.ctrlKey !==true) || event.keyCode === 86){
			     ...
			     * @comment modified from showToolVariations to bindClickEvent and showToolOnClickFunction
			     */
			    function bindClickEvent(){
			    	$('.formulacontainer').off('mouseenter mouseleave click').on('mouseenter mouseleave click','.formula-container',function(e){
			    		if(e.type === 'mouseenter'){
			    			$(this).addClass('addBoxShadow');
			    		}
			    		else if(e.type === 'mouseleave'){
			    			$(this).removeClass('addBoxShadow');
			    		}
			    		else if(e.type === 'click'){
			    			var $this = $(this),
			    				formulaTitle = $this.attr('formula'),
								toolUrl = "tools/"+formulaTitle+"/"+formulaTitle;
				    		require([toolUrl],function(){
								var args = {
									callback : eqeditorObj[formulaTitle].getVariationHtml(),
									addToparent : formulaTitle
								}
								showToolOnClickFunction(args);
				    		});
			    		}
					});
				}
			    
			    function showToolOnClickFunction(args){
					...
					var variationHtml = callback;
					$(variationHtml).insertAfter('.formulaselector');
					//var $this = $(this),
					var $this = $('.tool-bar-tools'+addToparent),
						$toolTip = $('.tool-tip'),
					...
					$this.parent().addClass('clickBoxShadow');
			    ...
			    this.bindClickEvent = bindClickEvent;
