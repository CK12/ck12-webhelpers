...
define("tinymce/tableplugin/TableGrid", [
    ...
    return function(editor, table) {
        ...
        function isCellSelected(cell) {
            return cell && (dom.hasClass(cell.elm, 'x-ck12-mce-item-selected') || cell == selectedCell);
        }
        ...
        function getSelectedRows() {
            ...
            each(table.rows, function(row) {
                each(row.cells, function(cell) {
                    if (dom.hasClass(cell, 'x-ck12-mce-item-selected') || (selectedCell && cell == selectedCell.elm)) {
                        rows.push(row);
                        return false;
                    }
                });
        ...
        function pasteRows(rows, before) {
            ...
            dom.removeClass(dom.select('td.x-ck12-mce-item-selected,th.x-ck12-mce-item-selected'), 'x-ck12-mce-item-selected');
        }
        ...
        function setEndCell(cell) {
            ...
            if (startPos && endPos) {
                ...
                for (y = startY; y <= maxY; y++) {
                    for (x = startX; x <= maxX; x++) {
                        if (grid[y][x]) {
                            dom.addClass(grid[y][x].elm, 'x-ck12-mce-item-selected');
                        }
                    }
                }
            }
            ...
            editor.on('keydown', function(e) {
                if ((e.keyCode == VK.DELETE || e.keyCode == VK.BACKSPACE) && !e.isDefaultPrevented()) {
                    ...
                    if (table) {
                        ...
                        while (i--) {
                            if (!editor.dom.hasClass(cells[i], 'x-ck12-mce-item-selected')) {
                                return;
                            }
                        }
    ...
    return function(editor) {
        ...
        function clear(force) {
            ...
            if (force || hasCellSelection) {
                editor.dom.removeClass(
                    editor.dom.select('td.x-ck12-mce-item-selected,th.x-ck12-mce-item-selected'),
                    'x-ck12-mce-item-selected'
                );
        ...
        editor.on('MouseUp', function() {
            ...
            if (startCell) {
                ...
                selectedCells = dom.select('td.x-ck12-mce-item-selected,th.x-ck12-mce-item-selected');
                if (selectedCells.length > 0) {
                    ...
                    do {
                        if (node.nodeName == 'TD' || node.nodeName == 'TH') {
                            if (!dom.hasClass(node, 'x-ck12-mce-item-selected')) {
                                break;
                            }
...
define("tinymce/tableplugin/Dialogs", [
    ...
    return function(editor) {
        ...
        self.table = function(isProps) {
            ...
            function onSubmitTableForm() {
                ...
                editor.undoManager.transact(function() {
                    var borderWidth   = data.border ? 1 : 0,
                        inlineClasses = ['x-ck12-float', 'x-ck12-nofloat'];

                    if (data.inline) {
                        if( data['class'].indexOf(inlineClasses[1]) > -1 ) {
                            data['class'] = data['class'].replace(inlineClasses[1], inlineClasses[0]);
                        } else {
                            data['class'] = inlineClasses[0];
                        }
                    } else {
                        if( data['class'].indexOf(inlineClasses[0]) > -1 ) {
                            data['class'] = data['class'].replace(inlineClasses[0], inlineClasses[1]);
                        } else {
                            data['class'] = inlineClasses[1];
                        }
                    }
                    ...
                    if (editor.settings.table_style_by_css) {
                        stylesToMerge = [];
                        stylesToMerge.push({name: 'border', value: borderWidth});
                        mergeStyles(dom, tableElm, stylesToMerge);
                        dom.setAttribs(tableElm, {
                            'data-mce-border': borderWidth
                        });
                        if (tableElm.children) {
                            for (var i = 0; i < tableElm.children.length; i++) {
                                styleTDTH(tableElm.children[i], 'border', borderWidth);
                            }
                        }
                    } else {
                        editor.dom.setAttribs(tableElm, {
            ...
            if (isProps === true) {
                ...
                if (tableElm) {
                    data = {
                        border: !!parseInt( dom.getAttrib(tableElm, 'border') ),
                        inline: dom.hasClass(tableElm, 'x-ck12-float'),
                        summary: dom.getAttrib(tableElm, 'summary') || '',
                ...
                items: [
                    {
                        ...
                        items: [
                            colsCtrl,
                            rowsCtrl,
                            {label: 'Summary', name: 'summary', maxWidth: 400},
                            {label: 'Inline', name: 'inline', type: 'checkbox'},
        ...
        self.cell = function() {
            ...
            cells = editor.dom.select('td.x-ck12-mce-item-selected,th.x-ck12-mce-item-selected');
        ...
        self.row = function() {
            ...
            each(tableElm.rows, function(row) {
                each(row.cells, function(cell) {
                    if (dom.hasClass(cell, 'x-ck12-mce-item-selected') || cell == cellElm) {
                        rows.push(row);
                        return false;
                    }
                });
...
define("tinymce/tableplugin/Plugin", [
    ...
    function Plugin(editor) {
        ...
        function handleDisabledState(ctrl, selector, visibleState) {

            function bindStateListener() {
                var selectionStart = editor.selection.getStart(),
                    hasParent      = editor.dom.getParent(selectionStart, selector);

                ctrl.disabled(!hasParent);
                if(visibleState){
                    ctrl.visible(hasParent)
                }
        ...
        function handleStateInsertTable(){
            var ctrl = this;

            function bindStateListener() {
                editor.on('nodechange', function(e){
                    var currentNode = editor.selection.getNode(),
                        isTable     = !!(currentNode.nodeName === 'TABLE' || editor.dom.getParent(currentNode, 'table.mce-item-table'));

                    // ctrl.disabled(isTable);
                    ctrl.visible(!isTable);
                });
            }
        ...
        function handleDisabledStateCM(){
            var ctrl = this;

            function handleDisable (e) {
                var table = editor.dom.getParent(e.element, 'table'),
                    th    = editor.dom.getParent(e.element, 'th'),
                    tr    = editor.dom.getParent(e.element, 'tr'),
                    disabledState = ( isElementBox(tr) || isElementBox(th) );

                if(!table){ disabledState = true; }

                ctrl.disabled( disabledState );
                ctrl.visible( !disabledState );
                if(!disabledState){
                    editor.fire('contextmenu:override', { enable: [
                        'ck12table',
                        'ck12link:link',
                        'ck12image:insert'
                    ]});
            ...
            function isElementBox(node){
                return !!(node && node.className && node.className.indexOf('x-ck12-element-box') > -1);
            }
        ...
        function postRenderCM() {
            /*jshint validthis:true*/
            handleDisabledStateCM.call(this);
        }
        ...
        if (editor.settings.table_grid === false) {
            ...
            editor.addMenuItem('inserttable', {
                ...
                onPostRender: handleStateInsertTable,
                menu: [
                    {
                        type: 'container',
        ...
        editor.addMenuItem('tablepropsCM', {
            text: 'Table properties',
            context: 'insert',
            onPostRender: postRenderCM,
            onclick: dialogs.tableProps
        });
        ...
        editor.addMenuItem('deletetableCM', {
            text: 'Delete table',
            context: 'insert',
            onPostRender: postRenderCM,
            cmd: 'mceTableDelete'
        ...
        editor.on('nodechange', function() {
            ...
            n = editor.selection.getStart();
            p = editor.dom.getParents(n, 'td,th,caption');

            // Bug 10025 n.nodeName.className check is added.
            if( n.nodeName === 'TABLE' && (n.nodeName.className && n.nodeName.className.indexOf('x-ck12-element-box') === -1)) {
                if (!!p){
                    editor.fire('active', { button: 'ck12table'});
                } else {
                    editor.fire('inactive', { button: 'ck12table'})
                }
            }
        ...
        each({
            ...
            mceTableMergeCells: function(grid) {
                ...
                if (!editor.dom.select('td.x-ck12-mce-item-selected,th.x-ck12-mce-item-selected').length) {
    ...
    PluginManager.add('ck12table', Plugin);
});
})(this);
