...
tinymce.ThemeManager.add('ck12modern', function(editor) {
	 ...
	 */
	function createToolbars(args) {
		var toolbars = [],
			classes  = (args && args.classes) ? args.classes + ' toolbar-grp' : 'toolbar-grp',
			border   = (args && args.border) ? args.border : '0 0 0 0';
	 ...
	 * @return {Object} Name/value object with theme data.
	 */
	function renderInlineIframeUI(args) {
		var panel,
			container,
			toolbar,
			$toolbar,
		...
		panel = self.panel = Factory.create({
			type: 'panel',
			role: 'application',
            classes: 'tinymce tinymce-inline hide',
            layout: 'stack',
            style: 'visibility: hidden',
            autohide: false,
            autofix: false,
		...
		editor.fire('BeforeRenderUI');
		panel.renderBefore(args.targetNode.nextSibling || args.targetNode).reflow();
		...
		function reposition() {
			...
			if(!toolbar){
				toolbar  = container.querySelector('.mce-ck12-inline-toolbar');
				$toolbar = $(toolbar);
				$parent  = $toolbar.parents('.mc-editor-row, .bordered-container').first();
				borderWidth = $parent.css('border-width') ?  parseInt($parent.css('border-width').slice(0, -2)) : 2;
				parentBorderWidth = borderWidth * 2;
			}

			height = parentBorderWidth ? parentBorderWidth + $toolbar.height() : $toolbar.height();
        ...
        var resizeInlineIframe = debounce(function resizeInlineIframe(){
            var ed   = tinymce.activeEditor,
                body = ed.getBody();

            var bodyHeight = body.clientHeight;
            if (bodyHeight < ed.settings.height){ bodyHeight = ed.settings.height; }

            bodyHeight += 'px';

            ed.editorContainer.style.height = bodyHeight;
            ed.iframeElement.style.height   = bodyHeight;
        }, 500);

		editor.on('focus', function(){
			if(!container) { container = editor.getContainer(); }
			reposition();
			container.classList.remove('mce-hide');
		});

		editor.on('blur hide', function(){
			if(!container) { container = editor.getContainer(); }
			container.classList.add('mce-hide');
		});

        editor.on('change nodechange', resizeInlineIframe);
	...
	self.renderUI = function(args) {
		...
		} else if (settings.inline === 'hybrid'){
			return renderInlineIframeUI(args);
		}
