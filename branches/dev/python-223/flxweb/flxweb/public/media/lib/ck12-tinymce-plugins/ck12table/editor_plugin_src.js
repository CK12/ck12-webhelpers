...
(function(tinymce) {
    ...
    tinymce.create('tinymce.plugins.TablePlugin', {
        init : function(ed, url) {
            ...
            if(ed.settings.ck12_plugins_url) {
                url = ed.settings.ck12_plugins_url + 'ck12table';
            }
            ...
            ed.onInit.add(function(ed) {
                ...
                ed.plugins.ck12table.fixTableCellSelection=fixTableCellSelection;

                // Add context menu
                if (ed && ed.plugins.contextmenu) {
                    ed.plugins.contextmenu.onContextMenu.add(function(th, m, e) {
                        var sm, se = ed.selection, el = se.getNode() || ed.getBody();
                        var td = ed.dom.getParent(e, 'td'); 
                        var th = ed.dom.getParent(e, 'th');
                        var tr = ed.dom.getParent(e, 'tr');
                        
                        if((tr && tr.className && tr.className.indexOf('x-ck12-element-box') != -1) || (th && th.className && th.className.indexOf('x-ck12-element-box') != -1) ) {
                        // element box table, do not show table context menu.
                        
                        } else if (td || th || ed.dom.select('td.mceSelected,th.mceSelected').length) {
                            ...
                            //Bug 10712  for math image within table, context menu should show edit equation. 
                            if (el.nodeName == 'IMG' && /^(x-ck12-block-math|x-ck12-math)$/.test(el.className)) {
                                m.add({title: 'Edit equation', icon: 'image', cmd: 'mceMathImage', ui: true });
                                m.addSeparator();
                            }else if (el.nodeName == 'IMG' && el.className.indexOf('mceItem') == -1) {
                                m.add({title : 'advanced.image_desc', icon : 'image', cmd : ed.plugins.advimage ? 'mceAdvImage' : 'mceImage', ui : true});
                                m.add({title : 'Delete Image', icon : 'delete', cmd : 'mceDeleteImage', ui : true});
                                m.addSeparator();
                            }
    ...
    // Register plugin
    tinymce.PluginManager.add('ck12table', tinymce.plugins.TablePlugin);
})(tinymce);
