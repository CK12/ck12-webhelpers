...
(function() {
    ...
    var ELEMENT_BOX_CLASS = 'x-ck12-element-box';

    var lastSelectedRng, t, helpers;
    ...
    tinymce.create('tinymce.plugins.CK12EmbedPlugin', {
        ...
        init : function(ed, url) {
            ...
            ed.on('preinit', function() {
                ...
                ed.parser.addNodeFilter('iframe', function(nodes) {
                    ...
                    while(i--){
                        ...
                        self.replaceNode(node.parent, imgnode);
                    }
                });
            ...
            ed.on('nodechange', function(e) {
                var n = e.element,
                    disableMedia = false;
                 //Disable media when inside Table or Element box

                if(ed && ed.parents && ed.parents.length > 0) {
                    // Loop over parents array as there could be other elements than 'TD'
                    for (var i = 0;
                        (i < ed.parents.length && !disableMedia); i++) {
                        var arrElm = ed.parents[i];
                        if ('TD' === arrElm.tagName.toUpperCase()) {
                            disableMedia = true;
                        } else if ('DIV' === arrElm.tagName.toUpperCase() && arrElm.className.indexOf(ELEMENT_BOX_CLASS) !== -1) {
                            disableMedia = true;
                        }
                    }
                }
                if( !disableMedia && (n.nodeName === 'TD' || (n.nodeName === 'DIV' && n.className.indexOf(ELEMENT_BOX_CLASS) !== -1))) {
                    disableMedia = true;
                }

                //Bug 7931 : disable Insert/Edit Media option for headings. It should only be available for paragraphs.
                if (null !== n.parentNode) {
                    if (n.parentNode.nodeName !== 'P') {
                        if (n.nodeName !== 'P' || n.parentNode.nodeName !== 'BODY') {
                            disableMedia = true;
                        }
                        // Bug 9912 on IE P is not present at the start unless user click somewhere in editor.
                        if (tinymce.isIE && n.nodeName === 'BODY' && n.parentNode.nodeName === 'HTML') {
                            disableMedia = false;
                        }
                    }
                }
                ...
                if (disableMedia) {
                    ed.fire('inactive', { button: 'ck12embed' });
                    ed.fire('disable', { button: 'ck12embed' });

                } else if (helpers.isImage(n)){
                    if(helpers.isEmbeddedObject(n)){
                        ed.fire('active', { button: 'ck12embed' });
                        ed.fire('enable', { button: 'ck12embed' });
                    } else {
                        // is just an image
                        ed.fire('inactive', { button: 'ck12embed' });
                        ed.fire('disable', { button: 'ck12embed' });
            ...
            ed.on('init', function() {
                if (ed && (ed.plugins.contextmenu || ed.plugins.ck12contextmenu) ) {
                    ed.addMenuItem('embedObjectEditCM', {
                        text: 'Edit Embedded',
                        context: 'insert',
                        cmd: 'mceEmbedDialog',
                        icon: 'media',
                        onPostRender: function () {
                            self.contextMenu.postRender.call(this, ed);
                        },
                        ui: true
                    });

                    ed.addMenuItem('embedObjectDeleteCM', {
                        text: 'Delete Embedded',
                        context: 'insert',
                        cmd: 'mceEmbedDelete',
                        image: url + '/images/icon_obj_del.gif',
            ...
            helpers = (function() {
                ...
                function isImage(node){
                    return truthy(node) && isEql(node.nodeName, 'IMG');
                };

                function isParagraph(node){
                    return truthy(node) && isEql(node.nodeName, 'P');
                };

                function isImageWithEmbedClass(node){
                    return isImage(node) && ed.dom.hasClass(node, EMBED_PLACEHOLDER_CLASS);
                }

                function getImageWithinParagraph(node) {
                    return (isParagraph(node) && isImage(node.firstElementChild)) ? node.firstElementChild : node;
                }

                function isEmbeddedObject(node){
                    return isImageWithEmbedClass(node) || isImageWithEmbedClass(getImageWithinParagraph(node));
                }

                function getEmbeddedObject(node){
                    node = getImageWithinParagraph(node);
                    return isEmbeddedObject(node) ? node : null;
                }

                return {
                    isEmbeddedObject: isEmbeddedObject,
                    getEmbeddedObject: getEmbeddedObject,
                    isImage: isImage
                };
            }());
            ...
            $.extend(true, ed.events, {
                'ck12embed': {
                    isEmbeddedObject: helpers.isEmbeddedObject
                }
            });
        },
       contextMenu: {
            postRender: function(editor) {
                ...
                function handleDisable () {
                    var el, disabledState;
                    t.setSelection(editor);
                    el = editor.selection.getNode();
                    el = helpers.getEmbeddedObject(el);
                    ...
                    if(disabledState) {
                        // Force selection of placeholder when contextmenu is triggered on parent
                        // otherwise contextmenu will have no target when triggered on the right
                        // half of the dom placeholder
                        setTimeout(function(){
                            $(el).click();
                        }, 0);
                        editor.fire('contextmenu:override', { enable: ['ck12embed'] });
                    }
                }
        ...
        replaceNode: function (oldNode, newNode) {
            // Using the default replace without using a try/catch will have
            // the node being replaced throw an error stating the prev
            // attribute is null.
            try {
