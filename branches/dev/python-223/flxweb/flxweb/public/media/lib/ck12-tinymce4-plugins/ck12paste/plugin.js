...
/* globals tinymce:false, $:false, tinyMCE:false */
(function () {
    'use strict';
    ...
    tinymce.create('tinymce.plugins.CK12PastePlugin', {
        init: function (ed) {
            ...
            var t = this,
                acceptableID = ['__mce_tmp', 'ck12image', '__mce_tmp_caption', 'previewImg', 'mcepastebin'],
            ...
            ed.addCommand('mcePreProcess', function (ui, o) {
                ...
                // To create link
                linkMatch = content.match(/^(https?:\/\/|ssh:\/\/|ftp:\/\/|file:\/|www\.)(.+)$/i);
                if(linkMatch){
                	if (linkMatch[1] == 'www.') {
                		linkMatch[1] = 'http://www.';
    				}
                	replaceLink = linkMatch[1]+(linkMatch[2].split(' '))[0];
                	content = '<a href="'+replaceLink+'">'+replaceLink+'</a>';
                }
                ...
                content = content.replace(/<\/?\s*br\s*(class="Apple-interchange-newline")?\/?>/g, '\n');
                ...
                newContent.find('img.x-ck12-block-math').each(function(){
                	var $this = $(this);
                	if($this.attr('alt') !== undefined){
                		$this.attr('alt',$this.attr('alt').replace("<","\\lt ").replace(">","\\gt "));
                	}
                	if($this.attr('data-flx-url') !== undefined){
                		$this.attr('src',$this.attr('data-flx-url'));
                	}
                });
                    ...
                    for (i = 0; i < imgArray.length; i++) {
                        ...
                        if (src !== null) {
                            ...
                            if (-1 !== src.toString().indexOf('data:image')) {
                            	...
                            	if (flxurl_data.indexOf('/flx/show') === 0 || flxurl_data.indexOf('/flx/math') === 0 || flxurl_data.indexOf('/flx/render') === 0 || flxurl_data.indexOf('/media') === 0){
                            		formattedImg = '<div class="x-ck12-img-postcard x-ck12-nofloat" ><p><img src ="' + flxurl_data + '"';
                            	}
                            	else {
                ...
                if (content.indexOf('MathJax-Span') !== -1) {
                    if (content.indexOf('x-ck12-mathEditor') !== -1) {
                    	...
                    	content = (content.indexOf("<span") == 0 ? t._pasteEquation(tinyMCE.activeEditor, ("&nbsp;"+content+"&nbsp;")):t._pasteEquation(tinyMCE.activeEditor, content));
                    } else {
                        content = '';
                    }
                }
                ...
                var youtubeCheck = content.indexOf('youtube.com/watch?v='),
                    youtubeIdx = (youtubeCheck != -1) ? youtubeCheck : content.indexOf('youtu.be/');
                if(youtubeIdx !== -1){
                	var youtubeId = (youtubeCheck != -1) ? $.trim(content.substring(youtubeIdx+20, youtubeIdx+31)) : $.trim(content.substring(youtubeIdx+9, youtubeIdx+20));
                	if(youtubeId.length === 11){
                		content = t._youtubeEmbed(tinyMCE.activeEditor, content, youtubeId);
                	}
                }

                // Add spans to beginning and end to prevent content from being cut off when close
                // to another element with a tag
                if(tinymce.isIE) { content = '<span></span>' + content + '<span></span>'; }
                o.content = content;
            });
        ...
        _pasteEquation: function (editor, content) {
            ...
            $.each((contentUpdate.find('.x-ck12-mathEditor')), function () {
                ...
                $this.html(thisContent).attr('id', newId).removeClass('selectedElement showContextMenu forSingleClick doubleClick');
        ...
        _youtubeEmbed: function (editor, content, youtubeId) {
        	var modifiedContent = '<iframe width="560" height="315" src="https://www.youtube.com/embed/' + youtubeId + '" frameborder="0" allowfullscreen></iframe>';
        	$.ajax({
                url : '/ajax/create/embeddedobject/',
                data : {
                    'embed_code' : b64EncodeUnicode(modifiedContent)
                },
                success : function(data){
                	if(data) {
                        if(data.status !== 'error'){
                            var ed = top.tinymce.activeEditor,
                            	img_node =  ed.plugins.ck12embed.htmlstrToImg(data.iframe),
                            	img_attr = {},
                            	iframe_id = ed.dom.getAttrib(ed.selection.getNode(), 'data-ck12embed-iframe-id');
