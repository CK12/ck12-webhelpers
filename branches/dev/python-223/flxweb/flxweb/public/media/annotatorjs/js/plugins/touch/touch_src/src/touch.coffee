...
class Annotator.Plugin.Touch extends Annotator.Plugin
  ...
  constructor: (element, options) ->
    super
    @colors = options.colors #ck12
    @utils = Annotator.Plugin.Touch.utils
    @createAdderTemplate() #ck12
    @selection = null
  ...
  pluginInit: ->
    ...
    @document.delegate(".annotator-hl", "tap", preventDefault: true, @_onHighlightTap)
  ...
  _setupControls: ->
    ...
    @adder = @controls.find(".action-annotate")
    ...
    @colorPicker = @controls.find(".circle")
    @colorPicker.on("tap", @_onPickerTap)
  
    #listener for eidtor delete
    @annotator.editor.subscribe('delete', @onAnnotationDelete)
  ...
  _setupAnnotatorEvents: ->
    ...
    @editor = new Touch.Editor(@annotator.editor, @colors) #ck12 pass colors to editor
  ...
  _onHighlightTap: (event) =>
    ...
    if jQuery.contains(@element[0], event.currentTarget)
      anno = $(event.currentTarget).data('annotation'); #ck12
      @editor.updateColor(anno.highlightColor)#ck12
      if anno.length != 0
        event.stopPropagation()
        @document.unbind("tap", @_onDocumentTap)
        @annotator.onEditAnnotation(anno) #ck12
        return false
      else
  ...
  #This function is a callback of color picker on the Adder
  _onPickerTap: (event) =>
    color = $(event.currentTarget).attr('data-id') #ck12
    event.preventDefault()
    if @range
      browserRange = new Annotator.Range.BrowserRange(@range)
      range = browserRange.normalize().limit(@element[0])
      ...
      if range and not @annotator.isAnnotator(range.commonAncestor)
        ...
        onAnnotationCreated = (annotation) =>
          ...
          annotation.quote= range.toString()
          annotation.ranges = [range]
          annotation.highlightColor = color  #ck12 Selected color will be stored in the annotation
        ...
        # and triggering the correct events for persistance.
        @annotator.onAdderClick(event)
        @annotator.editor.submit(event)  #ck12: this function call will save the annotation and make the editor invisible
        # @element.find('annotator-touch-adder-menu').removeClass('annotator-touch-hide')
        @hideControls()
  #When annotation is created, we need to update the color of selected text.
  setAnnotationHighlights: (annotation) =>
    if !annotation.highlightColor
      annotation.highlightColor = @colors[0] 
    $(annotation.highlights).addClass(annotation.highlightColor)
  #We use this function to initialize the color of the highlights when the annotations' data is received from the server.
  setAnnotationsHighlights: (annotations) =>
    for annotation in annotations
      if annotation.text.length != 0
        $(annotation.highlights).first().prepend('<i class="annotation-icon icon-bookmark"></i>').find('.annotation-icon').css('color', @colors[annotation.highlightColor] || @colors['orange'])
      @setAnnotationHighlights(annotation)
  
  #We use this function to update the color of the editor if the use click 'Annotate' on the adder
  #The default color of the editor is the first element in the color array.
  setEditorColor: (editor, annotation) =>
    @editor.updateColor(annotation.highlightColor || @colors[0]) if @editor and @colors.length >= 0

  onAnnotationDelete: (annotation)=>
    $(annotation.highlights).first().find('.annotation-icon').remove()
    @annotator.deleteAnnotation(annotation)

  #The function creates a template for adder based on the colors
  createAdderTemplate: ()=>
    colorTemplate = ''
    for color in @colors
      colorTemplate += '<div class="circle '+color+'" data-id="'+color+'"></div>'
    @template = """
     <div class="annotator-touch-adder-menu annotator-touch-hide">
