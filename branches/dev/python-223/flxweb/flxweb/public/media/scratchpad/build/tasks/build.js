module.exports = function( grunt ) {
  ...
  function convert( name, path, contents ) {
    ...
    if ( /.\/var\//.test( path ) ) {
      ...
      // Ignore scratchpad's exports (the only necessary one)
      if ( name !== "scratchpad" ) {
      } 
      console.log("THE NAME OF THE FILE BEING PROCESSED: ", name);
      ...
      switch ( name ) {
        case 'scratchpad':
          ...
          break;
        case 'text':
          console.log("EXCLUDING THE text.js PLUGIN");
          contents = '';
          break;
        case 'templates/templates':
          console.log("PROCESSING TEMPLATES FILE: ", name);
    ...
    if ( (amdName = grunt.option( "amd" )) !== null && /^exports\/amd$/.test( name ) ) {
      if (amdName) {
        grunt.log.writeln( "Naming scratchpad with AMD name: " + amdName );
      } else {
        grunt.log.writeln( "AMD name now anonymous" );
      }
      // Remove the comma for anonymous defines
      contents = contents
        .replace( /(\s*)"scratchpad"(\,\s*)/, amdName ? "$1\"" + amdName + "\"$2" : "" );
    }
    return contents;
  }
  ...
  function buildTemplateModule(contents) {
    ...
    paths.forEach(function(path, index, array) {
      ...
      path = path.trim();
      var _absPath = process.env.PWD +'/'+ PATH_ROOT+path;
      console.log("OPENING: ", _absPath);
      body += vars[index] + ":" + "'" + fs.readFileSync(_absPath, {encoding:"utf8", flag:"r"})
