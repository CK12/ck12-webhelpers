 ...
 * $Id$
 */
define('flxweb.createworkbook.exercise',[
...
function($) {
    ...
    $.fn.addtocrow = function () {
        ...
        lastrow.find('.input_conceptid').val('');
        lastrow.find('.input_handle').val('');
        lastrow.find('.input_noq').val('');
    ...
    function renderWorkbookStatusCheck(json_status) {
        ...
        if (json_status!=null && json_status.taskId && json_status.taskServer ) {
            ...
            $.flxweb.showDialog("Thank you! The workbook you requested is being generated. Please wait for a moment to download the workbook, or we will email you when it is available for download.",{ 'title' : "Workbook Notification" });
        }
        data = {}
    ...
    function get_toc_data() {
        ...
        $('#workbooktocform').children(".toccontent").each(function (index) {
            ...
            var noq =  parseInt($.trim($(this).find('.input_noq').val()),10);
    ...
    function get_title() {
        return 'workbook_title=' + escapeHTML($.trim($('input[name=workbooktitle]').val()));
    }

    /* Converts extended ASCII codes into escape format */
    function escapeHTML(str){
        ...
        while (len--) {
            var iC = aStr[len].charCodeAt();
            if (iC >= 128 && iC < 256) {
                aRet.push('\\u00'+iC.toString(16).toUpperCase());
            } else {
                aRet.push(escape(aStr[len]).replace('%u','\\u'));
            }
        }
    ...
    function show_workbook_result(result) {
        ...
        for (each in workbook) {
            if (workbook[each].uri.length > 0 ) {  
                $('.workbookresultpanel').append('<a href="' + workbook[each].uri + '" target="_blank" >Download workbook in ' + workbook[each].worksheetType + '</a>');
            } 
        }
    ...
    function addContent(e, data) {
        ...
        addToWorkBookToc(artifactData)
            $.ajax({
            type: 'POST',
            url: webroot_url + 'ajax/exercise/questioncount/'+artifactData.encodedID,
            success: updateExerciseRow,
            dataType: 'json'
        });
        $.flxweb.editor.AddContentDialog.close();
    }

    function  updateExerciseRow(result) {
        exercise_row = exerciseInTOC(result.encodedID);
        if(exercise_row) {
            if (exercise_row.find('.input_noq').val().length == 0 || 
                exercise_row.find('.input_noq').val() == exercise_row.find('.max_questions').val() || 
                parseInt(exercise_row.find('.input_noq').val(),10) > result.questionCount
               ) { 
                exercise_row.find('.input_noq').val(result.questionCount);
            }
            exercise_row.find('.max_questions').val(result.questionCount);
            exercise_row.find('.maxquestions').html(result.questionCount);
        }
    } 

    function addToWorkBookToc(artifact) {
        exercise_row = exerciseInTOC(artifact.encodedID); 
        if (!exercise_row) { 
             exercise_row = getNextEmptyExerciseDom();
        } 
        exercise_row.find('.input_execisedetails').html( $.flxweb.truncate(artifact.title,45) );
        exercise_row.find('.input_conceptid').val(artifact.encodedID);
        exercise_row.find('.input_handle').val(artifact.handle);
        exercise_row.effect('pulsate');
    }

    function exerciseInTOC(encodedID) {
        var exercise_in_toc = $('#workbooktocform').find('input[name="conceptid"][value="'+encodedID+'"]').eq(0).parent();
        if (exercise_in_toc.hasClass('toccontent') && exercise_in_toc.is(':visible')) {
            return exercise_in_toc;
        } 
        return false; 
    }
    ...
    $.validator.addMethod("hasLimitedNOQ", function(value, element) {
        ...
        $('.input_noq').each(function(index) {
              noq = parseInt($(this).val(),10);
              maxNoq = $(this).parent().parent().find('.max_questions').val();
              if( isNaN(noq) || (noq <= 0 || noq > maxNoq) ) {
		 isGood = false;
                 return;  
              }
        });
    ...
    workbookForm = $('#createworkbookform').validate({
            ...
            messages : {
                ...
                noq           : { required : "Please fill up the number of questions in all the selected exercises", hasLimitedNOQ : "No. of questions must be with the range of maximum questions in all the selected exercises", digits : "No. of questions must be in numbers in all the selected exercises"} ,
