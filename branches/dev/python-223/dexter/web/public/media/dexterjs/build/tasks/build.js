module.exports = function( grunt ) {
    ...
    function convert( name, path, contents ) {
        ...
        if ( /.\/var\//.test( path ) ) {
            ...
            // Ignore dexterjs's exports (the only necessary one)
            if ( name !== "dexterjs" ) {
                contents = contents
        ...
        if ( (amdName = grunt.option( "amd" )) !== null && /^exports\/amd$/.test( name ) ) {
            if (amdName) {
                grunt.log.writeln( "Naming dexterjs with AMD name: " + amdName );
            } else {
                grunt.log.writeln( "AMD name now anonymous" );
            }
            // Remove the comma for anonymous defines
            contents = contents
                .replace( /(\s*)"dexterjs"(\,\s*)/, amdName ? "$1\"" + amdName + "\"$2" : "" );
