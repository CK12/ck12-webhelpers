...
class FixEmptyResource:
    ...
    def fix(self, memberID=None):
        ...
        for arid, aid in ids:
            ...
            oaid = aid
            #
            #  Check and see if it has resource.
            #
            ...
            if arhr:
                continue
            #
            #  No resource, try to copy from one of its previous
            #  revisions that do.
            #
            ...
            if len(revisions) > 1:
                ...
                for n in range(1, len(revisions)):
                    ...
                    if arhrs and len(arhrs) > 0:
                        #
                        #  Found a revision with resource.
                        #
                        i = n
                        while i > 0:
                            i -= 1
...
if __name__ == "__main__":
    ...
    if verbose:
        print('Fixing artifacts with empty resource', end='')
        if memberID:
            print('for member with id %s' % memberID)
        print('')

    c = FixEmptyResource(url, verbose)
    c.fix(memberID=memberID)
