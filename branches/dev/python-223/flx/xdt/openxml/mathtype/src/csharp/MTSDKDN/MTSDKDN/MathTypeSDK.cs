...
namespace MTSDKDN
{
    ...
    public class MathTypeSDK
    {
        ...
        public static extern int RegisterClipboardFormatA(string lpString);
		...
		{
            const uint RPC_C_IMP_LEVEL_ANONYMOUS = 1;
            const uint RPC_C_IMP_LEVEL_IDENTIFY = 2;
            const uint RPC_C_IMP_LEVEL_IMPERSONATE = 3;
            const uint RPC_C_IMP_LEVEL_DELEGATE = 4;
            Ole32Methods.CoInitializeSecurity(IntPtr.Zero, -1, IntPtr.Zero, IntPtr.Zero, (uint)1, RPC_C_IMP_LEVEL_DELEGATE, IntPtr.Zero, (uint)0x0040, IntPtr.Zero);
			...
			if (hResult != 0) {
                Console.WriteLine("Error calling CoCreateInstance()");
				return null;
            }
