...
class parse_lines(object):
    ...
    def analyze(self, lines):
        ...
        while startpos<len(lines)-1:
            ...
            if prefix==':':
                ...
                node = T(type=T.t_complex_tag, tagname="ol", liststyle=lines[startpos].liststyle)
                newitem = lambda: T(type=T.t_complex_tag, tagname="li", blocknode=True)
                endtag = "ol"
            elif prefix==';':
                node = T(type=T.t_complex_style, caption=';')
    ...
    def preprocess_list(self,lines):
        new_lines = []
        for each in lines:
            if each.lineprefix.startswith('#'):
                prefix = '#'
                count_of_prefix = each.lineprefix.count(prefix)
                     
                if count_of_prefix == 2: 
                    each.liststyle ="x-ck12-lower-alpha"
                elif count_of_prefix == 3: 
                    each.liststyle ="x-ck12-lower-roman"
                else:
                    each.liststyle ="x-ck12-decimal"
        ...
        while i<len(self.tokens):
            ...
            if t.type in (T.t_item, T.t_colon):
                ...
                if startline is not None:
                    ...
                    startline=None
                if lines:
                    lines = self.preprocess_list(lines)
                    self.analyze(lines)
