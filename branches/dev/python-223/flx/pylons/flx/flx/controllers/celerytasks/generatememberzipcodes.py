...
class MemberZipcodeTask(GenericTask):
    """Class to generate Zipcode recommendations for members who have and updated Zipcode information.
    """
    recordToDB = False
    ...
    def __init__(self, **kwargs):
        ...
        self.csv_headers = ['Member ID', 'Zipcode', 'Deduce']
        self.output_file = '/tmp/member_zipcodes.csv'
    ...
    def run(self):
        """Prepares CSV file for Zipcode recommendations for members who have and updated Zipcode information.
        """
        members = dict()
        ...
        #Get all the exisitng memebrs who has updated there zipcodes.  
        ...
        for result in results:
            ...
            members[member_id] = zipcode
            csv_writer.writerow([member_id, zipcode, 'true'])
        log.info("Fetched all the existing members who has updated there zipcodes. Count: %s" % len(members))

        # Get all the groups of the users who has updated there zipcodes.
        ...
        # For every member get all the members in his groups and update there zipcode information.
        member_count = 0
        for member_id in members:
            log.info("Processing member %s:%s" % (member_id, str(members[member_id])))
            zipcode = members[member_id]
            ...
            for group_id in group_ids:
                ...
                for group_member_id in group_member_ids:
                    # Do not add the user if it already has location information.
                    if not members.get(group_member_id):
                        csv_writer.writerow([group_member_id, zipcode, 'false'])
                        member_count += 1
        fd.close()
        log.info("Completed collecting all the Members zipcode data.")
        log.info("Members having zipcode information: %s" % len(members))
        log.info("Members recommended for zipcode information: %s" % member_count)
