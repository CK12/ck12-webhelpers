...
class Http:
    ...
    def call(self, durl, method='GET', params=None, usePostBody=True):
        """
                ...
                if method == 'POST':
                    if usePostBody:
                        ...
                        if params:
                            ...
                            params = None
                    r = requests.post(durl, data=params, headers=headers, cookies=cj, timeout=self.timeout, allow_redirects=False)
                elif method == 'GET':
                    r = requests.get(durl, params=params, headers=headers, cookies=cj, timeout=self.timeout, allow_redirects=False)
                elif method == 'DELETE':
                    r = requests.delete(durl, data=params, headers=headers, cookies=cj, timeout=self.timeout, allow_redirects=False)
                elif method == 'PUT':
                    r = requests.put(durl, params=params, headers=headers, cookies=cj, timeout=self.timeout, allow_redirects=False)
                else:
