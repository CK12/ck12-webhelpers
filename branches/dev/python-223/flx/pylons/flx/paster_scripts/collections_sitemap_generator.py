...
from flx.controllers.artifact import ArtifactController
...
COLLECTION_API = "/collection/collectionHandle=$COLLECTION_HANDLE$"
...
def getPracticeDetailUrl(branchName, modality_url, conceptCollectionHandle):
    return "%s/%s/%s/%s&isPageView=true&collectionCreatorID=3&iscollectionurl=true&conceptCollectionHandle=%s&collectionHandle=%s"%(SERVER_URL,ASMT_URL_PREFIX,branchName,modality_url,conceptCollectionHandle,branchName)


def run(CollectionHandle=None,otherBranchName=None,ChangeFreqDef="monthly",Priority=0.8,OutputFileName=None, debug=False, checkURLS=True):
    """
    Create sitemap for Branch BranchEID='MAT.ARI'
    """
    # take parameters
    if not CollectionHandle and not otherBranchName:
...
***USAGE***
    ...
    data_list = getSiteMapData(CollectionHandle,otherBranchName, debug=debug, checkURLS=checkURLS)
...
def getSiteMapData(CollectionHandle,otherBranchName, debug=False, checkURLS=True):
    """
    ...
    if CollectionHandle:
        #branchName = CollectionHandle
        branchName = CollectionHandle
        try:    
            collections_api = COLLECTION_API.replace("$COLLECTION_HANDLE$", CollectionHandle)
            log.info("collections_api:: %s." % (collections_api))
            
            if collections_api:
                paramsDict = {'collectionCreatorID':3, 'includeRelations':'True', 'expirationAge':'daily'}
                response = remoteapi._makeCall(TAXONOMY_SERVER, collections_api, 500, params_dict=paramsDict, method='GET')
                collection = response['response']['collection']
                if collection:
                    # Skip collection that are not published
                    if not collection['isPublished']:
                        return
                    #creating branch url 
                    branchName = ((collection['handle']).replace('&','&amp;'))
        ...
        try:
            ...
            def processContains(containsEntry):
                #if hasattr(containsEntry,'contains'):
                if containsEntry.has_key('contains'):
                    for entry in containsEntry['contains']:
                        processContains(entry)
                if containsEntry.has_key('handle') and containsEntry.has_key('encodedID'):
                    conceptHandleERR = containsEntry['handle']
                    eid = containsEntry['encodedID']
    ...
    #                        raise Exception('No such concept node by EID or handle: %s' % eid)
                    ...
                    try:
                        modalities = api.getRelatedArtifactsForDomains(domainIDs=[domain.id], ownedBy='ck12',conceptCollectionHandle='%s-::-'%(CollectionHandle),conceptCollectionHandleLikeQuery=True, collectionCreatorID=3)
                        if len(modalities)>0:
                            #putting concept url data into list
                            conceptHandle = (containsEntry['handle'].replace('%s-::-'%CollectionHandle,''))
                            summary['total_urls'] +=1
                            values.append({
                                ...
                                if rec.artifactTypeID in newArtifactTypesDict:
                                    ...
                                    if modality_type not in EXCLUDE_MODALITY_TYPES:
                                        ...
                                        if entry_type == 'practice':
                                            _url = getPracticeDetailUrl(branchName.lower().replace(' ','-'), modality_handle, rec.conceptCollectionHandle).replace('&','&amp;')
                                        summary['total_urls'] +=1
            ...
            for topContains in collection['contains']:
                processContains(topContains)
