...
#               2) sudo paster shell
#               3) from paster_scripts import generate_all_pdfs
#               4) generate_all_pdfs.start(app, url)
...
LOG_FILENAME = "/opt/2.0/log/allpdfs.log"
log = logging.getLogger('GeneratAllPDF')
...
def start(app, url):
    ...
    #publishedBooksIDs = publishedBooksIDs[:94]
    #publishedBooksIDs = publishedBooksIDs[80:83]
    publishedBooksIDs = [7174]
    count = 0
    successCount = 0
    failureCount = 0
    ...
    for eachPublishedBook in publishedBooksIDs:
        ...
        printToLog('%d. Generating PDF - artifactID: %s, name: %s' %(count, eachPublishedBook, artifact.name))
        #renderPDFEndPoint = 'http://localhost/flx/render/pdfnoauth/%s/nocache/onecolumn/' %(eachPublishedBook)
        #renderPDFEndPoint = 'http://localhost/flx/render/pdfnoauth/%s/nocache/twocolumn/' %(eachPublishedBook)

        startTime = datetime.now()
        try:
            response = app.get(url(controller = 'pdf', action =
                                   'render',id=eachPublishedBook,
                                   revisionID=None,
                                   nocache=True,template='onecolumn'),headers={'Cookie': loginHandler.getLoginCookie(1)})
        ...
        if task.status == 'SUCCESS':
            ...
            printToLog('\t PDF generation completed for artifactID: %s \
                   \n\t DownloadUri: %s \
                   \n\t Elapsed Time: %s' %(eachPublishedBook, downloadUri, elapsedTime))
            successCount = successCount + 1
        elif task.status == 'FAILURE':
            printToLog('\t PDF generation failed for artifactID: %s \
                   \n\t Error Message: %s \
