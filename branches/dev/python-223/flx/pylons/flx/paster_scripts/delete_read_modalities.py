...
LOG_FILE_PATH = "/tmp/delete_modality.log"
        ...
        try:
            ...
            if cons:
                ...
                for con in cons:
                    ...
                    if artifact:
                        # If modality exist delete the modality.
                        logger.info("Deleting Read modalities for eid/artifactID %s/%s" %(eid, artifact.id))
                        api.deleteArtifactByID(artifact.id)                        
                    else:
                        logger.info("Read modalities with internal-tag does not exists for eid:%s" %eid)
