from auth.tests import TestController, url
...
class TestMemberController(TestController):
    ...
    def test_logout(self):
        member = api.getMemberByLogin(login='guest')
        response = self.app.get(url(controller='member', action='logout'), params = {'sessionID': member.id, 'returnTo': '/flx/login/memberForm'}, headers={'Cookie': self.getLoginCookie(1)})
    ...
    def test_getMembers(self):
        members = api.getMembers(sorting='lastLogin,desc', filterDict={ 'adminOnly': False }, pageNum=1, pageSize=100)
        assert members, "No member found"
        members = api.getMembers(searchDict={ 'gender': 'male' }, pageNum=1, pageSize=100)
        assert members, "No member found"
        members = api.getMembers(searchDict={ 'firstName': 'CK12' }, pageNum=1, pageSize=100)
        assert members, "No member found"
    ...
    def test_createMember(self):
        ...
        assert '"status": %d' % ErrorCodes.INVALID_EMAIL in response, "Expect error with invalid email"

        memberParams['email'] = 'rogerrabbit@ck12.org',
        response = self.app.post(url(controller='member', action='create'), params = memberParams, headers={'Cookie': self.getLoginCookie(1)})
