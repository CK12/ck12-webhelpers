...
class GenericTask(Task):
    ...
    def initialize_db(self):
        ...
        if not dbsession:
            dblock.acquire()
            try:
                from auth.model import init_model, getSQLAlchemyEngines as getEngines
    ...
    @classmethod
    def enqueueTask(cls, asyncResult, kwargs):
        try:
            if cls.recordToDB:
                with lock:
                    taskID = asyncResult.task_id
                    logger.info("enqueueTask: RMQ Task ID: %s" % taskID)
                    hasSession = kwargs.get('hasSession', False)
                    if hasSession:
                    ...
                    if not task:
                        ...
                        if hasSession:
                            session = meta.Session
                            task = api._createTask(session, **taskDict)
                        else:
    ...
    def updateTask(self):
        """
            Update the task with additional attribute values.
            Note: This method does not change the state of the task to
                  keep it in sync with the Celery/RabbitMQ states.
        """
        try:
